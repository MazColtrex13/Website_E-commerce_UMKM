...
class PagesController extends Controller
{
    ...
    public function viewproduct($id)
    {
        ...
        // Integrate with RajaOngkir to get provinces
        $rajaOngkirController = new \App\Http\Controllers\Api\RajaOngkirController();
        $provinceResponse = json_decode($rajaOngkirController->getProvince(), true);
        $provinces = isset($provinceResponse['rajaongkir']['results']) ? $provinceResponse['rajaongkir']['results'] : [];

        // Get cities for the first province as default
        $cities = [];
        if (!empty($provinces)) {
            $cities = json_decode($rajaOngkirController->getCity($provinces[0]['province_id']), true)['rajaongkir']['results'];
        }

        return view('viewproduct', compact('product', 'relatedproducts', 'reviews', 'averageRating', 'relatedProductRatings', 'provinces', 'cities'));
    }
    ...
    public function getCities(Request $request)
    {
        ...
        $rajaOngkirController = new \App\Http\Controllers\Api\RajaOngkirController();
        $cityResponse = json_decode($rajaOngkirController->getCity($provinceId), true);
        $cities = isset($cityResponse['rajaongkir']['results']) ? $cityResponse['rajaongkir']['results'] : [];
    ...
    {
        $rajaOngkirController = new \App\Http\Controllers\Api\RajaOngkirController();
        $costResponse = json_decode($rajaOngkirController->getCost($request), true);
        $costs = isset($costResponse['rajaongkir']['results']) ? $costResponse['rajaongkir']['results'] : [];
