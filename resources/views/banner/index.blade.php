...
@section('content')
  <div class="flex flex-col w-full max-h-screen overflow-y-auto ">
        ...
        <div class="flex items-center justify-around w-full mt-2 lg:justify-between">
            ...
            <a href="{{ route('banner.create') }}" class="px-3 py-1 text-white bg-yellow-500 rounded-lg ">Tambahkan Banner</a>
        </div>
                                ...
                                <td class="px-4 py-2">
                                    <div class="flex flex-col space-y-2 sm:flex-row sm:space-y-0 sm:space-x-2">
                                        ...
                                        <a class="w-full px-4 py-2 text-xs font-bold text-white bg-[#9a031f] rounded cursor-pointer sm:w-auto sm:text-sm md:text-base lg:text-sm"
                                            onclick="showpopup({{ $banner->id }})">Hapus</a>
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="fixed inset-0 items-center justify-center hidden bg-gray-600 bg-opacity-50 backdrop-blur-sm"
            ...
            <form action="{{ route('banner.destroy', $banner->id) }}"
                ...
                <p class="mb-6 text-gray-600">Apakah Anda yakin ingin menghapus kategori ini? <br> <span
                        class="text-red-500">This action cannot be undone.</span></p>
                <input type="hidden" name="dataid" id="banner_id">
                <div class="flex justify-center space-x-4">
                    <button type="submit"
                        class="px-6 py-2 text-white bg-red-600 rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50">Ya,
                        Hapus</button>
                    <button type="button" onclick="hidepopup()"
                        class="px-6 py-2 text-white bg-gray-600 rounded-lg hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-opacity-50">Batal</button>
                </div>
            </form>
        </div>
        @endforeach
